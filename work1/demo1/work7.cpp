...
using namespace std;        //如果用下标来判断指针是否为空,不能用是否是0来判断
                            //因为0也是指针的一部分索引

char* bfstrstr(const char*str, const char* substr)
{
    char* dest = (char*) str, *sub = (char*) substr;
    while(*sub&&*dest) //字串
    {
        if(*dest != *sub)
        {
...
const char* bfstrstr1(const char* str,const char* substr)
{
    size_t i=0 ,j=0;
 ...
 */
int* kmpNext(const char* patternStr) {
    //将 patternStr 转为 字符数组形式
  

    size_t plen = strlen(patternStr);
    //预先创建一个next数组，用于存储部分匹配表的每个子串的最长公共前后缀
    int* next = new int(plen); //要记得delete
...
const char* kmpstrstr(const char* str, const char* substr)          //kmp算法
{
...
int main()
{
    ...
    if(p) cout << "str中存在substr,首地址开始的字符串为:" << p <<endl;
    else cout <<  "str中不存在substr!" <<endl;

    //if(kmpstrstr(str,0)==0) cout <<"ok" <<endl;
}